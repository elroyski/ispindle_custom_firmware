...
## Kompilacja w PlatformIO

Aby skompilować projekt w środowisku PlatformIO:

1. Zainstaluj [PlatformIO Core](https://platformio.org/install/cli) lub [PlatformIO IDE](https://platformio.org/install/ide) (jako wtyczkę do VSCode)
2. Sklonuj to repozytorium:
   ```
   git clone https://github.com/elroyski/ispindle_custom_firmware.git
   ```
3. Przejdź do katalogu projektu:
   ```
   cd ispindle_custom_firmware
   ```
4. Uruchom kompilację:
   ```
   pio run
   ```
5. Aby wgrać program do urządzenia:
   ```
   pio run -t upload
   ```

Projekt zawiera kilka środowisk kompilacji zdefiniowanych w pliku `platformio.ini`:
- `d1_mini_git` - domyślne środowisko z numerem wersji git
- `d1_mini_standalone` - wersja bez numeru git
- `d1_mini_git_stage` - wersja używająca najnowszej platformy espressif8266
